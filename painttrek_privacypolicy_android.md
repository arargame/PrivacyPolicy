# Paint Trek – Privacy Policy (Android)

**Last Updated: 22 September 2026**

This privacy policy explains what data the mobile game **Paint Trek** (package name `com.arargame.PaintTrek.Android`, "the Game") processes, why, and what choices you have.
...
The Game uses **Google Analytics for Firebase** (Firebase Analytics) to understand gameplay performance, balance difficulty levels, and improve player experience.

Data processed includes:
- **Anonymous Gameplay Events:** Level started, level completed, level failed, first level completed (`first_level_completed`), level 2 milestone (`level_2_reached`), weapon crates collected (`weapon_collected`), diamonds collected (`diamond_collected`), boss fight outcomes (`boss_fight_result`), and rewarded ad revive responses (`second_chance_used`).
- **Screen Transitions (`screen_view`):** Navigated menus and screens (such as `MainMenuScreen`, `GameBoard`, `PauseScreen`, `GameOverScreen`).
- **Anonymous User Properties:** Preferred game language (`selected_language`), progression stage bucket (`progress_bucket`), level milestone (`has_passed_level_1`), unlocked level (`max_unlocked_level`), and device performance tier (`device_tier`).
- **Pseudonymous Player Identifier:** A randomly generated local GUID assigned upon first installation (`settings.PlayerId`). This identifier is completely random and contains no personally identifiable data.
...
The Game offers optional digital purchases processed **exclusively through Google Play Billing** (e.g., unlocking game modes, extra lives, level unlocks, and developer support).
...
Your game progress, high scores, audio volume settings, and configurations are stored locally on your device in private application storage (`GameSettings`).
...
- You can enable or disable notifications at any time via Android Settings → Apps → Paint Trek → Notifications.
